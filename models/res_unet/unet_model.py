...
#         self.sfss = [SaveFeatures(base_layers[i]) for i in range(8)]
        self.up1 = UnetBlock(512, 256, 256)
        self.up2 = UnetBlock(256, 128, 256)
        self.up3 = UnetBlock(256, 64, 256)
        self.up4 = UnetBlock(256, 64, 256)

        self.up5_1 = nn.Sequential(nn.ConvTranspose2d(256, 256, 2, stride=2),
                                 nn.BatchNorm2d(256),
                                 nn.ReLU(),
                                 nn.Conv2d(256, self.num_classes, 1),)

        self.up5_2 = nn.Sequential(nn.ConvTranspose2d(256, 256, 2, stride=2),
                                 nn.BatchNorm2d(256),
                                 nn.ReLU(),
                                 nn.Conv2d(256, self.num_classes, 1),)
...
#         [print(self.sfss[i].features.shape) for i in range(8)]
        '''~~~ 0: Decoder ~~~'''
        ...
        fea = x
        output_1 = self.up5_1(x)
        output_2 = self.up5_2(x)
        '''~~~ 0: ENDs ~~~'''

        return output_1, output_2, fea
